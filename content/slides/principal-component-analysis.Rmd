...
## Population PCA {.allowframebreaks}
  ...
  - Dimension reduction method: 
    + Let $\mathbf{Y} = (Y_1, \ldots, Y_p)$ be a random vector with covariance matrix $\Sigma$. We are looking for a transformation $h: \mathbb{R}^p \to \mathbb{R}^k$, with $k\ll p$ such that $f(\mathbf{Y})$ retains "as much information as possible" about $\mathbf{Y}$.
  - In PCA, we are looking for a **linear transformation** $f(y) = w^Ty$ with **maximal variance** (where $\|w\|=1$)
...
## Properties of PCA {.allowframebreaks}
  ...
  - We have
  $$\sum_{i=1}^p \lambda_i = \sum_{i=1}^p \mathrm{Var}(w_i^T\mathbf{Y}) = \sum_{i=1}^p \mathrm{Var}(\mathbf{Y}).$$
