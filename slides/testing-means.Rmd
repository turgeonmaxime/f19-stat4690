...
## Test for a multivariate mean: $\Sigma$ known
  ...
  $$ n(\bar{\mathbf{Y}} - \mu)^T\Sigma^{-1}(\bar{\mathbf{Y}} - \mu) > \chi^2_{\alpha}(p).$$
...
## Test for a multivariate mean: $\Sigma$ unknown {.allowframebreaks}
  ...
  $$ T^2=n(\bar{\mathbf{Y}} - \mu)^TS_n^{-1}(\bar{\mathbf{Y}} - \mu) > c,$$
...
<!--   - Let $a\in\mathbb{R}^p$. We are interested in constructing confidence intervals for $a^T\mu$ that are simultaneously valid (i.e. right coverage probability) for all $a$. -->
